#!/usr/bin/env python3
"""
rl_server_m0_0.py  —  RL server / Model M0_0

State (3):  [coverage_n, new_edges_n, crashes_n]
DQN:        Linear(3→128→128→64→47)
SHM:        128 B  /tmp/rl_shm_m0_0
"""
...
SHM_SIZE        = 128
MODEL_PATH      = os.environ.get("RL_M0_0_MODEL_PATH", "rl_m0_0.pt")
...
# ── Action labels (must match mutator_m0_0.c) ─────────────────────────────────
ACTION_COLUMNS = [
...
# exploitation phase before training ends. This is set dynamically in main().
...
MAX_CRASHES         = 1000.0
STEP_COST           = 0.1
PLATEAU_WINDOW      = 10_000  # wider window to avoid false-positive plateaus
PLATEAU_MIN_DELTA   = 1
...
class DQNAgent:
    ...
    def train_step(self):
        ...
        # Entropy regularisation: encourage the network to spread probability
        # mass across actions, preventing premature collapse to a single action.
        ...
        if self._train_steps % TARGET_SYNC == 0:
            ...
            print(f"[i] M0_0 target sync @ {self._train_steps}")
        return td_loss.item()
    ...
    def save(self, path):
        ...
        print(f"[+] M0_0 saved → {path}")

    def load(self, path):
        if not os.path.exists(path): print(f"[i] M0_0: no checkpoint — fresh."); return
        ...
        self._train_steps = ck.get("train_steps", 0); self._total_steps = ck.get("total_steps", 0)
        print(f"[+] M0_0 loaded {path}  (ε={self.epsilon:.3f}, steps={self._total_steps})")
...
def shm_read(shm):
    ...
    return (struct.unpack_from("=I", raw, STATE_SEQ_OFF)[0],
            struct.unpack_from("=I", raw, COVERAGE_OFF)[0],
            struct.unpack_from("=I", raw, NEW_EDGES_OFF)[0],
            struct.unpack_from("=I", raw, CRASHES_OFF)[0],
            struct.unpack_from("=Q", raw, TOTAL_EXECS_OFF)[0])
...
def build_state(cov, ne, cr):
    return np.array([cov / MAX_COVERAGE,
                     min(ne, MAX_NEW_EDGES) / MAX_NEW_EDGES,
                     math.log1p(cr) / math.log1p(MAX_CRASHES)], dtype=np.float32)
...
def main():
    ...
    mpath = os.path.join(args.results_dir,
                         f"rl_metrics_m0_0_{'eval' if is_eval else 'train'}.csv")

    print(f"[+] M0_0  mode={tag}  state={STATE_SIZE}  actions={ACTION_SIZE}  "
    ...
    with open(mpath, "w") as f:
        f.write("step,reward,coverage_term,crash_term,loss,epsilon,coverage,crashes,action\n")

    print(f"[+] M0_0 ready.  metrics={mpath}")

    pcov=0; pcr=0; pact=46; pstate=build_state(0,0,0)
    last_seq=0; aseq=1; step=0; cov_start=None
    stop="user interrupt"; cov=0; cr=0

    try:
        while True:
            if is_eval and step >= args.eval_steps:   stop=f"eval limit";   break
            if not is_eval and step >= args.train_steps: stop=f"train limit"; break
            ...
            last_seq=cur; _, cov, ne, cr, _ = shm_read(shm)
            if is_eval and cov_start is None: cov_start=cov

            state=build_state(cov, ne, cr)
            ...
            if step % 100 == 0:
                with open(mpath,"a") as f:
                    f.write(f"{step},{rew:.4f},{comps['coverage_term']:.4f},"
                            ...
                            f"{cov},{cr},{act}\n")
                print(f"[M0_0-{tag}:{step:>7}] cov={cov:<5} ne={ne:<3} cr={cr} "
