#!/usr/bin/env python3
"""
rl_server_m1_0.py  —  RL server / Model M1_0

Full edge stability distribution state (over all 65536 edges).

State (12):
  [0]  coverage_n            = coverage / 65536
  [1]  new_edges_n           = min(new_edges,100) / 100
  [2]  crashes_n             = log1p(crashes) / log1p(1000)
  [3]  enabled_mean_n        = mean(enabled[i]) / train_steps
  [4]  enabled_std_n         = std(enabled[i])  / train_steps
  [5]  enabled_max_n         = max(enabled[i])  / train_steps
  [6]  enabled_nonzero_frac  = count(en[i]>0)   / total_edges
  [7]  disabled_mean_n       = mean(disabled[i]) / train_steps
  [8]  disabled_std_n        = std(disabled[i])  / train_steps
  [9]  disabled_max_n        = max(disabled[i])  / train_steps
  [10] disabled_nonzero_frac = count(dis[i]>0)  / total_edges
  [11] mean_stability_ratio  = sum(en/(en+dis+1)) / total_edges

  --train-steps N is the hard step limit AND the MAX_COUNT normaliser.
  Step 0 is skipped (C side seeds prev_trace and returns no SHM signal).

DQN:  Linear(12→128→128→64→47)
SHM:  256 B  /tmp/rl_shm_m1_0
"""
...
SHM_SIZE        = 256
MODEL_PATH      = os.environ.get("RL_M1_0_MODEL_PATH", "rl_m1_0.pt")
...
class DQNAgent:
    ...
    def train_step(self):
        ...
        if self._train_steps % TARGET_SYNC == 0:
            ...
            print(f"[i] M1_0 target sync @ {self._train_steps}")
        return td_loss.item()
    ...
    def save(self, path):
        ...
        print(f"[+] M1_0 saved → {path}")

    def load(self, path):
        if not os.path.exists(path): print("[i] M1_0: no checkpoint — fresh."); return
        ...
        print(f"[+] M1_0 loaded {path}  (ε={self.epsilon:.3f}, steps={self._total_steps})")
...
def main():
    ...
    ap.add_argument("--train-steps", type=int, default=DEFAULT_TRAIN_STEPS,
                    help="Step limit AND MAX_COUNT normaliser for enabled/disabled stats")
    ...
    mpath = os.path.join(args.results_dir,
                         f"rl_metrics_m1_0_{'eval' if is_eval else 'train'}.csv")

    print(f"[+] M1_0  mode={tag}  state={STATE_SIZE}  actions={ACTION_SIZE}  "
    ...
    zero_d = {"coverage":0,"new_edges":0,"crashes":0,"n_nz_en":0,"n_nz_dis":0,
              ...
              "sum_sq_dis":0,"sum_stab":0.0,"total_edges":1}
    with open(mpath, "w") as f:
        f.write("step,reward,coverage_term,crash_term,loss,epsilon,"
                "coverage,crashes,action,en_mean_n,dis_mean_n,stability\n")

    print(f"[+] M1_0 ready.  metrics={mpath}")
            ...
            if step % 100 == 0:
                ...
                with open(mpath,"a") as f:
                    f.write(f"{step},{rew:.4f},{comps['coverage_term']:.4f},"
                            ...
                            f"{cov},{cr},{act},{enm:.4f},{dim:.4f},{stb:.4f}\n")
                print(f"[M1_0-{tag}:{step:>7}] cov={cov:<5} ne={ne:<3} cr={cr} "
                      f"act={act:<2}({ACTION_COLUMNS[act]:<30}) "
                      f"ε={agent.epsilon:.3f} r={rew:+.2f} loss={loss:.5f} "
                      f"en={enm:.3f} dis={dim:.3f} stab={stb:.3f}")
