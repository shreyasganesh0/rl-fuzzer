#!/usr/bin/env bash
# scripts/run_m1_0.sh  —  train + eval pipeline for Model M1_0
# Run from repo root:  bash scripts/run_m1_0.sh [OPTIONS]
#
# Options:
...
MUTATOR_SO="${REPO_ROOT}/bin/mutator_m1_0.so"
MODEL_PT="${REPO_ROOT}/bin/rl_m1_0.pt"
AFL_TRAIN_DIR="${REPO_ROOT}/outputs/m1_0"
AFL_EVAL_DIR="${REPO_ROOT}/outputs_eval/m1_0"
PLOTS_DIR="${REPO_ROOT}/plots/m1_0"
...
if [[ $NO_BUILD -eq 0 ]]; then
    log "Compiling src/mutator_m1_0.c → $MUTATOR_SO"
    clang -O2 -shared -fPIC -I"${AFL_INC}" \
        -o "$MUTATOR_SO" "${REPO_ROOT}/src/mutator_m1_0.c"
...
if [[ $EVAL_ONLY -eq 0 ]]; then
    ...
    python3 "${REPO_ROOT}/scripts/rl_server_m1_0.py" \
