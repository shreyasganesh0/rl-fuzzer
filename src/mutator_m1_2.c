/*
 * mutator_m1_2.c  —  RL mutator / Model M1_2
 *
 * Input-buffer-aware edge stability model.
 *
 * Extends M1_1 (visited-edge stability, 13-dim state) with 51 input buffer
 * feature dimensions computed from the current test case:
 *   - buf_len_norm:      log1p(buf_size) / log1p(1MB)
 *   - entropy_norm:      Shannon entropy of byte distribution / 8.0
 *   - printable_ratio:   fraction of printable ASCII bytes
 *   - histogram[16]:     16-bin byte value histogram, normalized by buf_size
 *   - first_32_bytes[32]: first 32 bytes / 255.0, zero-padded
 *
 * Total state: 13 (M1_1) + 51 (buffer features) = 64 dimensions.
 *
 * SHM layout (512 bytes, /tmp/rl_shm_m1_2):
 *
 *   STATE REGION [0..255]
 *     [0]   state_seq     u32
 *     [4]   coverage      u32
 ...
 *     [12]  crashes       u32
 *     [24]  total_execs   u64
 *     [32]  n_nonzero_en  u32   count(en[i]>0) among visited
 *     [36]  n_nonzero_dis u32   count(dis[i]>0) among visited
 *     [40]  max_en        u32
 *     [44]  max_dis       u32
 *     [48]  sum_en        u64   sum over visited
 *     [56]  sum_sq_en     u64
 *     [64]  sum_dis       u64
 *     [72]  sum_sq_dis    u64
 *     [80]  sum_stability f32   sum(en/(en+dis+1)) over visited
 *     [84]  total_edges   u32   = num_visited
 *     [88]  step_count    u32
 *     [92]  buf_len       u32   raw buffer length
 *     [96]  entropy       f32   Shannon entropy [0..8]
 *     [100] printable_ratio f32 [0..1]
 *     [104] histogram[16] u32[16]  raw counts per 16-byte bin
 *     [168] first_bytes[32] u8[32] first 32 bytes of input
 *     [200] _pad[56]
 *
 *   ACTION REGION [256..511]
 *     [256] action_seq    u32
 *     [260] action        i32
 *
 * State vector (64 elements, built in Python):
 *   [0..12]  same as M1_1 (visited-edge stability)
 *   [13]     buf_len_norm
 *   [14]     entropy_norm
 *   [15]     printable_ratio
 *   [16..31] histogram[16]
 *   [32..63] first_32_bytes[32]
 */
...
#define SHM_PATH      "/tmp/rl_shm_m1_2"
#define SHM_SIZE      512
...
#define OFF_TOTAL_EDGES  84   /* = num_visited for M1_2 */
#define OFF_STEP_COUNT   88

/* Buffer feature offsets */
#define OFF_BUF_LEN       92
#define OFF_ENTROPY        96
#define OFF_PRINTABLE     100
#define OFF_HISTOGRAM     104   /* u32[16], 64 bytes */
#define OFF_FIRST_BYTES   168   /* u8[32], 32 bytes */

/* Action region (starts at 256 for 512-byte SHM) */
#define OFF_ACTION_SEQ  256
#define OFF_ACTION      260
...
static inline volatile uint8_t *u8_at(void *b, size_t o) {
    return (volatile uint8_t *)((uint8_t*)b+o);
}

/*
 * shm_push_state_m1_2
 *
 * Same visited-edge pass as M1_1, plus buffer feature computation.
 */
                            ...
                            const uint8_t *buf, size_t buf_size)
{
    ...
    for (uint32_t i = 0; i < sz; i++) {
        ...
        /* Only include in stats if edge has been visited */
        if (en + dis == 0) continue;
    ...
    *u32_at(s, OFF_TOTAL_EDGES) = (num_visited > 0) ? num_visited : 1u;
    *u32_at(s, OFF_STEP_COUNT)  = m->step_count;

    /* ── Buffer features ──────────────────────────────────────────────────── */
    *u32_at(s, OFF_BUF_LEN) = (uint32_t)buf_size;

    /* Single pass: byte frequency, 16-bin histogram, and printable count */
    uint32_t freq[256];
    ...
    for (size_t i = 0; i < buf_size; i++) {
        ...
        freq[b]++;
        hist[b >> 4]++;
        printable_count += (b >= 0x20 && b <= 0x7E);
    }

    /* Shannon entropy of byte frequency distribution */
    float entropy = 0.0f;
    if (buf_size > 0) {
        float inv_n = 1.0f / (float)buf_size;
        for (int i = 0; i < 256; i++) {
            if (freq[i] > 0) {
                float p = (float)freq[i] * inv_n;
                entropy -= p * log2f(p);
            }
        }
    }
    *f32_at(s, OFF_ENTROPY) = entropy;

    /* Printable ratio */
    float p_ratio = (buf_size > 0) ? (float)printable_count / (float)buf_size : 0.0f;
    *f32_at(s, OFF_PRINTABLE) = p_ratio;

    /* Write 16-bin histogram to SHM */
    for (int i = 0; i < 16; i++) {
        *u32_at(s, OFF_HISTOGRAM + i * 4) = hist[i];
    }

    /* First 32 bytes, zero-padded */
    volatile uint8_t *fb = u8_at(s, OFF_FIRST_BYTES);
    size_t copy_len = (buf_size < 32) ? buf_size : 32;
...
my_mutator_t *afl_custom_init(afl_state_t *afl, unsigned int seed)
{
    ...
    if (m->shm_fd < 0) { perror("[-] M1_2 SHM open"); m->shm = NULL; return m; }
    if (ftruncate(m->shm_fd, SHM_SIZE) < 0) perror("[-] M1_2 SHM ftruncate");

    m->shm = mmap(NULL, SHM_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, m->shm_fd, 0);
    if (m->shm == MAP_FAILED) {
        perror("[-] M1_2 SHM mmap"); m->shm = NULL;
    } else {
        printf("[+] M1_2 mutator: SHM at %s  (visited-edge + input buf features)\n", SHM_PATH);
    }
                        ...
                        size_t max_size)
{
    ...
    if (m->shm) {
        ...
        shm_push_state(m, coverage, new_edges, crashes, total_execs, buf, buf_size);
