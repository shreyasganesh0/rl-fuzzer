/*
 * mutator_m1_1.c  —  RL mutator / Model M1_1
 *
 * Edge stability model — visited edges only.
 *
 * Identical to M1_0 in mechanics, but distribution statistics are computed
 * exclusively over edges that have been reached at least once:
 *   visited edge i := enabled[i] + disabled[i] > 0
 *
 * This eliminates the majority-zero bias present in M1_0: in M1_0, the
 * ~64K unvisited edges drag every mean toward zero.  M1_1 asks "given the
 * edges we have actually reached, how stable are they?" — a cleaner signal.
 *
 * Additionally, total_edges in SHM = num_visited (not MAP_SIZE), so Python
 * uses the correct denominator when normalising means and fractions.
 * A 13th state element (num_visited_norm) is added.
 *
 * SHM layout (256 bytes, /tmp/rl_shm_m1_1) — same offsets as M1_0:
 *
 *   STATE REGION [0..127]
 *     [0]   state_seq     u32
 *     [4]   coverage      u32
 ...
 *     [84]  total_edges   u32   = num_visited  (key difference from M1_0)
 *     [88]  step_count    u32
 *     [92]  _pad[36]
 *
 ...
 *     [132] action        i32
 *
 * State vector (13 elements, built in Python):
 *   [0..11] same as M1_0 but over visited-only set
...
#define SHM_PATH      "/tmp/rl_shm_m1_1"
#define SHM_SIZE      256
...
#define OFF_TOTAL_EDGES  84   /* = num_visited for M1_1 */
 ...
 * total_edges is set to num_visited, not MAP_SIZE.
 */
...
my_mutator_t *afl_custom_init(afl_state_t *afl, unsigned int seed)
{
    ...
    if (m->shm_fd < 0) { perror("[-] M1_1 SHM open"); m->shm = NULL; return m; }
    if (ftruncate(m->shm_fd, SHM_SIZE) < 0) perror("[-] M1_1 SHM ftruncate");

    m->shm = mmap(NULL, SHM_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, m->shm_fd, 0);
    if (m->shm == MAP_FAILED) {
        perror("[-] M1_1 SHM mmap"); m->shm = NULL;
    } else {
        printf("[+] M1_1 mutator: SHM at %s  (visited-only edge stats)\n", SHM_PATH);
    }
